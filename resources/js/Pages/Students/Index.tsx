...
// ─── Student form (no classroom_id — assigned from Classroom detail) ─────────
function StudentForm({ student, onClose }: { student?: Student; onClose: () => void }) {
    ...
    const submit = (e: FormEvent) => {
        ...
        setZodErrors({});
        if (isEdit) put(`/students/${student!.id}`, { onSuccess: onClose });
        else        post('/students', { onSuccess: onClose });
    };
        ...
        <form onSubmit={submit} className="space-y-6">
            ...
            <fieldset className="space-y-4">
                ...
                <FormField label="Nama Lengkap" htmlFor="name" error={err('name')}>
                    <TextField id="name" value={data.name} onChange={(e) => setData('name', e.target.value)} placeholder="Nama lengkap santri" />
                </FormField>
                <div className="grid grid-cols-2 gap-4">
                    <FormField label="Tempat Lahir" htmlFor="birth_place" error={err('birth_place')}>
                        <TextField id="birth_place" value={data.birth_place ?? ''} onChange={(e) => setData('birth_place', e.target.value)} placeholder="Jakarta" />
                    </FormField>
                    <FormField label="Tanggal Lahir" htmlFor="birth_date" error={err('birth_date')}>
                        <TextField id="birth_date" type="date" value={data.birth_date ?? ''} onChange={(e) => setData('birth_date', e.target.value)} />
                    </FormField>
                </div>
            </fieldset>
            ...
            <fieldset className="space-y-4 border-t border-outline-variant pt-4">
                ...
                <FormField label="No. HP Wali" htmlFor="wali_phone" error={err('wali_phone')}>
                    <TextField id="wali_phone" value={data.wali_phone ?? ''} onChange={(e) => setData('wali_phone', e.target.value)} placeholder="08123456789" />
                </FormField>
                <FormField label="Alamat" htmlFor="address" error={err('address')}>
                    <TextArea id="address" rows={3} value={data.address ?? ''} onChange={(e) => setData('address', e.target.value)} placeholder="Alamat lengkap santri" />
                </FormField>
            </fieldset>
...
function ImportModal({ onClose }: { onClose: () => void }) {
    ...
    const submit = (e: FormEvent) => {
        ...
        router.post('/students/import', form, {
            forceFormData: true,
            onSuccess: onClose,
    ...
    return (
        <form onSubmit={submit} className="space-y-5">
            ...
            <div className="flex items-center justify-between rounded-lg border border-outline-variant bg-surface-container-low px-4 py-3">
                <div>
                    <p className="text-body-sm font-semibold text-on-surface">Template CSV</p>
                    <p className="text-xs text-on-surface-variant">Download lalu isi sesuai format</p>
                </div>
                <a
                    href="/students/template"
                    download
                    className="flex items-center gap-2 rounded-lg bg-surface-container-high px-3 py-2 text-xs font-semibold text-on-surface transition-colors hover:bg-primary/10 hover:text-primary"
                >
            ...
            {/* Format hint */}
            <div className="rounded-lg bg-primary/5 px-4 py-3 text-xs text-on-surface-variant">
                <p className="mb-1 font-semibold text-on-surface">Kolom CSV:</p>
                <code className="font-mono">nis, name, gender (L/P), birth_date, birth_place, address, wali_phone</code>
                <p className="mt-1">Kolom 4–7 opsional. Format tanggal: <code className="font-mono">YYYY-MM-DD</code></p>
            </div>

            {/* File input */}
            <div>
                <label className="block text-label-caps font-semibold uppercase tracking-wider text-on-surface-variant">
                    File CSV
                </label>
                <div
                    className="mt-1.5 flex cursor-pointer flex-col items-center justify-center rounded-lg border-2 border-dashed border-outline-variant bg-surface-container-low px-4 py-8 transition-colors hover:border-primary hover:bg-primary/5"
...
export default function StudentsIndex() {
    ...
    useDebounceEffect(debouncedSearch, () =>
        router.get('/students', { search: debouncedSearch, classroom_id: classroomId }, { preserveState: true, replace: true })
    );

    const openCreate = () => { setEditItem(undefined); setCrudModal(true); };
    const openEdit   = (s: Student) => { setEditItem(s); setCrudModal(true); };
    ...
    return (
        <AuthenticatedLayout header="Database Santri">
            ...
            {/* CRUD Modal */}
            <CrudModal show={crudModal} title={editItem ? `Edit ${editItem.name}` : 'Tambah Santri Baru'} onClose={() => setCrudModal(false)} maxWidth="xl">
                <StudentForm key={editItem?.id ?? 'create'} student={editItem} onClose={() => setCrudModal(false)} />
            </CrudModal>

            {/* Import Modal */}
            <CrudModal show={importModal} title="Import Data Santri" onClose={() => setImportModal(false)}>
                <ImportModal onClose={() => setImportModal(false)} />
            </CrudModal>

            <PageHero icon="group" title="Database Santri" subtitle="Kelola data akademik dan profil santri secara real-time." />
            ...
            <section className="overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest shadow-sm">
                        ...
                        {canEdit && (
                            <>
                                <button type="button" onClick={() => setImportModal(true)}
                                    className="flex items-center gap-2 rounded-lg border border-primary px-4 py-2 text-button font-semibold text-primary transition-all hover:bg-primary/5">
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-surface-container-low text-on-surface-variant">
                            <tr>
                                ...
                                {(canEdit || canDelete) && <th className="px-6 py-4 text-right text-label-caps tracking-wider">Aksi</th>}
                            </tr>
                        </thead>
                                    ...
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                {s.birth_place && <p className="text-xs text-on-surface-variant">{s.birth_place}</p>}
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-6 py-4 text-body-sm text-on-surface-variant">{s.gender === 'L' ? 'Laki-laki' : 'Perempuan'}</td>
                                    <td className="px-6 py-4">
                                        {s.classroom ? (
                                            <span className="rounded-full bg-surface-container-high px-3 py-1 text-xs font-semibold text-on-surface-variant">{s.classroom.name}</span>
                                        ) : (
                                            <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">Belum ada kelas</span>
                                        )}
                                    </td>
                                    <td className="px-6 py-4 text-body-sm text-on-surface-variant">{s.wali_phone ?? '—'}</td>
                                    {(canEdit || canDelete) && (
                                            ...
                                            <div className="flex justify-end gap-1">
                                                ...
                                                {canDelete && (
                                                    <button type="button" onClick={() => handleDelete(s)}
                                                        className="flex h-9 w-9 items-center justify-center rounded-lg text-outline transition-all hover:bg-error/5 hover:text-error" aria-label="Hapus">
                ...
                {students.last_page > 1 && (
                    <Pagination meta={students} onPage={(page) => applyFilter({ page })} label="santri" />
                )}
            </section>
        </AuthenticatedLayout>
    );
